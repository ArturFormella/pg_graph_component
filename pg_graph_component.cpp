...
extern "C" {
  ...
  Datum graph_component_combine(PG_FUNCTION_ARGS);
  ...
  typedef struct GraphComponentState {
      HTAB *vertices;
      Vertex *next;
...
*/
  ...
  */
  ...
  graph_components_step_arr(PG_FUNCTION_ARGS) {
      ...
      if (PG_ARGISNULL(0)) {
        state = (GraphComponentState *) MemoryContextAlloc(agg_context, sizeof(GraphComponentState));
        HASHCTL ctlDistinct;
      ...
      deconstruct_array(currentArray, INT4OID, elemTypeWidth, elemTypeByValue, elemTypeAlignmentCode, &currentVals, &currentNulls, &currentLength);
  ...
  */
  ...
  get_connected_components(PG_FUNCTION_ARGS) {
      ...
      if (funcctx->call_cntr < funcctx->max_calls) {
        do {
          vertexEntry = (Vertex *) hash_seq_search(&input_text->hash_seq);
          if (!vertexEntry) {
        ...
        } while (vertexEntry && (vertexEntry->firstItem->nextItem == vertexEntry->firstItem) );
        if (vertexEntry) {
